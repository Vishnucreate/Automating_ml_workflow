...
openai.api_key = 'sk-None-QH9co9aHWzz0kN7YAugTT3BlbkFJGfY1YhY1J9D2FTpL1uwF'

# Function to clean a text column using GPT
def clean_text_column(dataframe, column_name):
    dataframe[column_name] = dataframe[column_name].apply(lambda x: clean_text_with_gpt(x))
    return dataframe

def clean_text_with_gpt(text):
    prompt = f"Correct the spelling and grammar of the following text:\n{text}"
